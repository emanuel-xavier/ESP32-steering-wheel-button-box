  ...
  _reconnectTimer = setTimeout(async () => {
    try {
      ...
      if (data.found) {
        ...
        scheduleReconnect(); // keep retrying silently
      }
    } catch (_) {
      scheduleReconnect();
    }
...
function populateForm(cfg) {
  ...
  onDirectBtnsToggle();
  const n = useDirectBtns ? rawN : 14;  // keep a sensible default in the hidden input
...
function readForm() {
  const cfg = {};
  // numButtons first — firmware's jsonToConfig reads JSON in order; putting it early
  // ensures it's never silently dropped if the ArduinoJson pool fills up late.
...
async function loadConfig() {
  try {
    ...
    if (!res.ok) {
      ...
      dbg('READ ERROR from device', err);
      // Empty read = characteristic not populated yet; populate form with
      // defaults so the UI is usable and the user can still save.
      if ((err.error || '').includes('empty')) {
        populateForm({});
        setStatus('Could not read config — showing defaults. You can still save.', 'info');
      } else {
        setStatus('Read failed: ' + (err.error || res.status), 'error');
      }
      return;
    }
...
async function saveConfig() {
  ...
  try {
    ...
    if (res.ok) {
      ...
      // handleDisconnect hides config and retries every 5 s automatically
      handleDisconnect();
...
async function clearCache() {
  ...
  try {
    const res = await fetch('/clearcache', { method: 'POST' });
    if (res.ok) {
...
let _loadedCfg      = null;   // set by populateForm; used by getButtonLabel
const _pressedBtns  = new Set();
const _recentBtns   = [];     // ordered list of recently pressed buttons (most recent last)
const MAX_LOG       = 50;

// Returns a display label for a 1-based gamepad button number.
// Encoder buttons are labelled "Enc1 CW", "Z2 CCW", etc. using the loaded config.
function getButtonLabel(n) {
  ...
  const encBase = numDirect + matButtons + 1; // first encoder button (1-based)
  if (n < encBase) return { label: String(n), enc: false };
  const idx = n - encBase; // 0-based index within encoder buttons
...
function startDebugStream() {
  if (_debugEvtSource) return;
  renderRecent();
  _debugEvtSource = new EventSource('/debug/events');
  _debugEvtSource.onmessage = (e) => {
    const { btn, evt } = JSON.parse(e.data);
  ...
  _debugEvtSource.onerror = () => {
    // Disable the browser's built-in auto-reconnect: close and null so we
    // control exactly when a new stream is opened (in onConnected / the
    // retry below), preventing two simultaneous SSE subscriptions in the
    // Go broker which would deliver every event twice.
    if (_debugEvtSource) { _debugEvtSource.close(); _debugEvtSource = null; }
    // Retry after a short delay in case this was a transient HTTP hiccup
    // (BLE still up).  The guard in startDebugStream prevents a double-open
    // if onConnected fires at the same time.
    setTimeout(() => {
      const card = document.getElementById('debugCard');
      if (card && !card.classList.contains('hidden')) startDebugStream();
    }, 3000);
  };
}
